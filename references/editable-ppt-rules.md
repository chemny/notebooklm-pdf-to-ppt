...
- High-value heading regions may use a narrow secondary OCR check when the primary OCR misses leading section markers, punctuation, or fragments. This repair must be limited to the heading band, recorded in `ocr_repairs`, and completed before background cleanup or PPTX rebuild.
- Secondary heading OCR must pass a strict format gate before replacement. Valid repairs should match known heading structures such as `一、标题 (Key Words)`. Reject candidates with stray symbols, malformed `Key` text, impossible mixed fragments, or trailing artifacts, and record them as rejected `ocr_repair_candidates`.
- If primary OCR finds trustworthy adjacent top-heading fragments but splits Chinese and English pieces, merge them by x-order into one heading and record `layout_repairs`. This is safer than accepting a noisy secondary OCR replacement.
